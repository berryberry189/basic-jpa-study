...
- 생명 주기가 있다
- 공유할 수 있다
  - 회원 엔티티가 있다면 다른 엔티티 에서 얼마든지 회원 엔티티를 참조할 수 있다 
...
- 공유하지 않는 것이 안전하다
  - 값을 복사해서 사용해야 한다
  - 불변(Immutable) 객체로 만드는 것이 안전하다 => 생성 시점 이후 절대 값을 변경할 수 없는 객체
  - 오직 하나의 주인만이 관리해야 한다
  - ⭐ 값 타입은 정말 값 타입이라고 판단될 때만 사용해야한다. 식별자가 필요하고 계속해서 값을 추적하고 구분하고 변경해야 한다면 엔티티로 만들어야 한다.
  
  #### 임베디드 타입(복합 값 타입)
  - 주로 기본 값 타입을 모아서 만들어서 복합 값 타입이라고도 함
  - @Embeddable : 정의한는 곳에 표시  /   @Embedded : 사용하는 곳에 표시
  - 기본 생성자가 필수다
  - @AttributeOverride : 같은 임베디드 타입을 2번 이상 사용하는 경우 매핑정보 재정의를 위하여 사용 ( 엔티티에 설정해야 함 )
  
  #### 값 타입과 불변 객체
  - 임베디드 타입 같은 값 타입을 여러 엔티티에서 공유하면 문제가 발생 => 값(인스턴스)을 복사 해서 사용해야함
  - 객체의 공유 참조는 피할 수 없다
  
  #### 값 타입 비교
  - 값타입을 비교할때는 ==(동일성)이 아니라, equals()(동등성) 비교를 해야한다
  - equals() 재정의하면 hashCode()도 재정의 하는 것이 안전
